...
final class BudgetReconciliationTests: XCTestCase {
    ...
    /// **テンプレート由来の実測値を、モデルと同じ錠の中に閉じ込める。**
    ///
    /// `generationPromptOverhead = 7` と `perMessageTemplateOverhead = 5` は、
    /// `toolDefinitionTokens = 499` と**同じ `container.prepare` の差分**から出た実測である。
    /// ところが 499 だけが非ゲートの錠（`ToolExecutionTests` の説明文の錠と同居）で守られ、
    /// **この2つには非ゲートの表明が1つも無かった** ── 値を見ているのは
    /// `EngineToolWiringTests`（`SOPHIA_TOOLTOKENS=1`）だけで、**既定では走らない。**
    /// R10 が禁じている形そのものが、R10 を制定したコミット群の中に残っていた。
    ///
    /// ## なぜ 499 の錠へ同居させなかったか
    ///
    /// 499 があの場所で機能するのは、**あれを古くする変更が同じ関数の中にあるから**である
    /// ── 説明文を書き換える人の目に、必ず数字が入る。
    ///
    /// **7 と 5 を古くするのは説明文ではない。チャットテンプレート、つまりモデルである。**
    /// 説明文の錠へ同居させると、(1) 説明文を直す人には無関係な数字が並び、
    /// (2) **モデルを差し替えた人はどちらも見ない。** 錠として偽物になる。
    ///
    /// だからモデルIDと同じ表明の中に置く。**モデルを差し替えた瞬間にここが赤くなる。**
    /// 赤を消す正しい手順は `make tooltokens` で測り直すことであって、数字を書き換えることではない。
    func testTemplateOverheadsAreLockedToTheModelTheyWereMeasuredOn() {
        XCTAssertEqual(
            SophiaDefaults.modelID, "mlx-community/Qwen3-8B-4bit",
            """
            モデルが変わっている。**下の2つの実測値は、このモデルのチャットテンプレートで
            測ったものなので、既に古い可能性がある。**
            `make tooltokens` で測り直すこと。数字を書き換えて緑にしないこと。
            """)

        XCTAssertEqual(
            Budget.generationPromptOverhead, 7,
            """
            テンプレート末尾の生成開始ぶんが 7 から変わっている。
            **これは `transcript(armed:)` の引き算に直接入っていて、
            縮約が「収まった」と判断する境界そのものである**（499 と同じ重み）。
            """)

        XCTAssertEqual(
            Budget.perMessageTemplateOverhead, 5,
            "1発言あたりの role / 区切りの固定分が 5 から変わっている")
    }

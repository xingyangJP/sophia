...
final class SystemPromptTests: XCTestCase {
    ...
    func testSelfRecognitionMatchesTheModelfile() throws {
        ...
        //
        // **上限を 80 → 105 に上げた（2026-08-17）。プロンプトは1文字も変えていない。**
        // 概算式が文字種別（日本語0.74 / ASCII0.25）になり、精度が上がった結果である
        // （旧式は一律0.5で、実測に対し32%過少だった。PROGRESS 発見19）。
        // **同じ文が 71 → 97 と見えるようになっただけで、実際のコストは最初から97だった。**
        //
        // **この失敗はテストが仕事をした例である。** 概算を直したら、
        // それに依存していた「+71トークン」という文書上の数字が全部古いと分かった。
        let tokens = [SophiaMessage.system(prompt)].estimatedTokenCount
        XCTAssertLessThanOrEqual(
            tokens, 105,
            "自己認識が概算105トークンを超えた。増やすなら BENCH_RESULTS に実測を残してから")
    }

...
final class ConversationFolder {
    ...
    /// **モデルに「いまどのフォルダが結び付いているか」を教える1行**（`idle` では nil）。
    ///
    /// > **2026-08-18、実機で必要だと分かった。記録を残す。**
    /// > ツール定義は「結び付いたフォルダからの相対パス」と書いてあるが、
    /// > **どのフォルダが結び付いているかは1文字も渡っていなかった。**
    /// > 実機で「Youtuber フォルダに何がある？」と聞くと、モデルは
    /// > `path="Youtuber"` を渡した ── **名前を下位フォルダだと解釈した。**
    /// > 根の中身は `path=""` である。**それを知る手段が無かった。**
    /// >
    /// > プローブでは問いかけに `~/Documents` と書いてあったので、この穴が露出しなかった。
    /// > **問いかけの側に答えが混ざっていると、欠けているものが見えない。**
    ///
    /// **ここは `tojson` を通らない**ので日本語で書いてよい
    /// （ツール定義側は `\uXXXX` に展開されて7倍になるが、system プロンプトは展開されない）。
    ///
    /// **短く保つこと。** `armed` の間、毎ターン払う。
    var boundFolderNotice: String? {
        guard let displayName else { return nil }
        // **短さは実測の結果である。**
        // 最初に書いた3文は **108トークン**あった ── 利用者に残るのが 33トークンしかない配分
        // （`SophiaDefaults.InputBudget`）で、毎ターン払うには高すぎる。
        // 落とした3文目（「名前を path に書くと失敗します」）は**失敗の説明であって指示ではない**。
        // 1文目と2文目で同じことが言えている。**書き足す前に費用を測ること。**
        return #"参照先のフォルダ「\#(displayName)」。このフォルダ自身は path="" で指します。"#
    }

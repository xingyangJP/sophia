...
enum SophiaDefaults {
    ...
    /// | ツール定義 322（`armed` の間） | 16.4節（定義側。実測は 2026-08-18） |
    /// | 固定の前置き 105 | 4.8節（自己認識側） |
    ///
    /// **322 + 105 + 600 = 1,027。ファイルを1つ読ませた時点で総額を超える。**
    ...
    /// | 項目 | | 出所 |
    /// |---|--:|---|
    /// | `total` | 1,000 | **DESIGN 2.2章。**実測からの内挿（連続使用時もプリフィル10秒以内） |
    /// | `fixedPreamble` | 105 | **実測**（2026-08-18 `make tooltokens` の `baseline`） |
    /// | `toolDefinitions` | 322 | **実測**（`SophiaDefaults.toolDefinitionTokens`）。`armed` の間だけ（16.2節） |
    /// | `singleRead` | 360 | **割り付け**（測っていない。理由は `ContextBudget.singleRead`） |
    ...
    ///
    /// 1. ツール定義（322 ＝ 総額の32%）をさらに削る。**16.4節の担当**
    ...
    /// **したがって「表に収まっている」は、実トークナイザで確かめるまで【未確認】である。**
    /// 確かめる道は既にある ── `TokenCounter.exact` を挿して測ること（第15章の宿題）。
    ///
    ...
    enum InputBudget {
        ...
        /// **`idle` でも必ず払う。** 内訳は分離していない（型コメントの但し書き）。
        static let fixedPreamble = 105

        /// ツール定義（16.2節）。**`armed` / `resolving` の間だけ毎ターン払う。実測 322。**
        ...
        ///
        /// > **system プロンプトは二重に引かれている。** `fixedPreamble` の中にも入っており、
        /// > `fit` は送信列の中でもう一度数える。**意図的に安全側へ倒してある** ──
        /// > 過大に見積もる誤りは「窓を狭める」で済み、過少の誤りは壁である
        /// > （`ContextWindow` の `reportedTotal` と同じ非対称）。
        static func transcript(armed: Bool) -> Int {
            total - fixedCost(armed: armed)
        }
    }
    ...
    ///
    /// # 実測（2026-08-18 / `make tooltokens`・実トークナイザ）
    ///
    /// | | トークン |
    /// |---|--:|
    /// | `idle`（tools 引数を書かない場合と**厳密一致**） | 105 |
    /// | `armed`（出荷する3つ） | 427 |
    /// | **差＝ここの値** | **322** ＝ 入力予算 1,000 の **32%** |
    ///
    /// 英語化前は 1,182（予算の118%）で、プリフィルは毎ターン 8.2秒だった。
    /// いまは全体427トークンで約2.7秒である。
    ///
    /// # 動かしてよい条件
    ///
    ...
    static let toolDefinitionTokens: Int = 322

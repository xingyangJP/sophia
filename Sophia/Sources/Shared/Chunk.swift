...
/// ## 1行しか持たない。それが要点である
///
/// `summary` は `ToolResult.bookmarkLine` ── **栞と同じ文字列**である
/// （`読んだ: notes.md（全412行のうち 1-80行）`）。同じ値から作るので、
/// **画面に出た文と、ターンの中で文脈に残る文が食い違わない。**
///
/// > **【事実の訂正 / 2026-08-19】ここには「履歴に残る栞と同じ文」と書いてあった。**
/// > **ターンをまたぐと往復は痕跡ごと消える。** 栞が残るのは**ターンの中だけ**である
/// > （`MLXEngine.compacted`）。**同じ文字列であることは真だが、
/// > 「履歴に残る」は偽だった。**
...
struct ToolActivity: Sendable, Equatable, Codable {
    ...
    /// 画面に出せる1行（＝栞と同じ文字列。**「履歴に残る」ではない** ── 上の訂正を読むこと）。
    var summary: String

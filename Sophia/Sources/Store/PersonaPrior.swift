...
struct PersonaBelief: Sendable, Equatable, Codable {
    ...
    /// **精度（確信度）で重みを付けて混ぜる。値は動くが、確信度は積み増さない。**
    ///
    /// 値の側を精度で重み付けするのは、**弱い材料が強い材料を同じ重みで薄めない**ため。
    /// 血液型（0.02）が、使用中の訂正（0.75）と同じ重さで混ざってはいけない。
    ///
    /// ## ⚠ 確信度は足し算にしない —— **`max` を取る**
    ///
    /// **最初は足していた。試験が落ちて分かった** ── 同じ領域の質問が2問あると
    /// **0.5 + 0.5 = 1.0 で関門（0.7）を越え、質問だけで振る舞いが変わる。**
    /// **14.13c が明示的に否定している形**である（「質問は事前分布であって証拠ではない」）。
    /// **血液型で塞いだ「積み増しで断定できる」穴が、質問の側に空いていた。**
    ///
    /// 統計としても、足すのは**独立な証拠**のときだけである。
    /// **同じ場面で連続して選ばせた12問は独立ではない** ── 同じ日の、同じ気分の、
    /// 同じ自己像から出た答えで、**方法そのものが共通の偏りを持つ。**
    ///
    /// > **確信度は、いちばん強い材料が決める。数で決まらない。**
    /// > 弱い材料を何回積んでも、強い材料1つには届かない。
    func blended(with other: PersonaBelief) -> PersonaBelief {
        ...
        return PersonaBelief(
            ...
            confidence: min(1.0, max(confidence, other.confidence)))
    }
}

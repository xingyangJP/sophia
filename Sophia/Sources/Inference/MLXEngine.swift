...
final class ModelDownloadStallWatch: @unchecked Sendable {
    ...
    func evaluate(
        ...
        if let observed {
            ...
            if !hasObservedDisk {
                // ## ⚠ この枝は、出荷の観測関数の下では通らない【2026-09-05・監督の指摘】
                //
                // `observeDownloadTemporaryBytes` は**ディレクトリが読めれば 0 を返す**
                // （nil は読めないときだけ）。したがって `init` の観測が成功し、
                // **武装は取得が始まる前に済んでいる。**
                // 下の2つの直し（欠陥3・欠陥5）は、**構造としては本物だが、
                // 出荷構成では発火しない。**
                //
                // **だから「これが事故の原因だった」とは書けない。** 残してあるのは
                // **観測手段を差し替えた日に効く防御**としてである ──
                // 観測が nil を返しうる形（別ボリューム・権限・遅延生成）に変えた瞬間、
                // ここが唯一の受け皿になる。**到達性は【未確認】。検証役が実測で決着させる。**
                //

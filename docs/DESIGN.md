...
**差の約4,550トークン（95%）は Open WebUI が自動注入したビルトインツール32個のJSONスキーマ**で、
...
| 目標 | 入力トークンの上限 |
|---|--:|
| プリフィル 5秒以内（冷間） | 約 1,100 |
| プリフィル 10秒以内（冷間） | 約 1,600 |
| プリフィル 10秒以内（連続使用時も守る） | **約 1,000〜1,200** |

**設計値は 1,000〜1,200トークン**を採る。冷間の数字で設計すると実利用で破綻するため。
日本語は概ね1文字≒0.5トークンなので、**利用者の入力＋システムプロンプト＋文脈で2,000〜2,400字**が目安。
...
| # | 条件 | 満たさない場合 |
|---|---|---|
| 1 | **思考モードが OFF** | 思考が1,075文字（約700トークン）流れる。生成7〜13.4 tok/s では本文の1文字目まで**50〜100秒**。入力を0にしても届かない |
| 2 | **入力が約1,000〜1,200トークン以内** | 4,786トークンならプリフィルだけで冷間32〜35秒、連続使用時40〜155秒 |
...
生成7〜13.4 tok/s で10秒間に出せるのは70〜134トークン＝日本語で50〜95文字程度。
短い一問一答なら収まるが、通常の回答には届かない。
...
| 8 | 機能追加のたびにプロンプトが肥大する | 入力4,786トークン＝プリフィル34秒。Open WebUI で実際に起きた | 送信トークン数を `GenerationStats.inputTokens` で常時記録し、UIに表示する。予算1,000〜1,200トークンを超えたら警告（第2.2章） |

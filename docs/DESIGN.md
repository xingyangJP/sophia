...
| ~~4~~ | **【解決 / 2026-08-18 実測】出荷する定義3つの実費用 = 1,182トークン（予算の118%）。英語なら324。下の但し書きを必ず読むこと** |
...
>
> #### 打ち手（**英語化が桁違いに効く。ただし未検証**）
>
> **同じ意味の英語で 324トークン（-73%）。** 全体では 1,287 → 429 で、
> プリフィルは 8.2秒 → **2.7秒**。他の案（引数を削る・ツールを減らす）は
> **どれも100トークン台の節約**にしかならず、桁が違う。
>
> > **【未確認】英語の定義で日本語の指示から正しく呼べるかは測っていない。**
> > 2026-08-18 のプローブは**英語の対照も 3/3** だったが、
> > あれは**英語で問いかけた**場合である。**「英語の定義 × 日本語の問いかけ」は未測定。**
> > **説明文はモデルの挙動を変える入力である**（16.4節）。
> > `make toolprobe` を英語の定義で流し直してから決めること。
>

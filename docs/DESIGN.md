...
| **R10** | **既定で走らない計測が、値の正しさを守る唯一の仕掛けになってはならない。** env ゲートで隔離してよいのは**重い実行**であって**一致の表明**ではない。**通常実行に残す表明は「文言」ではなく「文言と数字の結び付き」を固定すること** | 文言だけを固定した錠は、**錠を書き換えれば緑に戻せる**ので数字を守らない |

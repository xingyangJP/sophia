...
> **初版の「3〜5問」は感覚だった。2026-09-05 に利用者判断で撤回した。**
> この製品の価値は設定の短さではなく、**一人を深く知ること**にある。
> 初回は、仕事の進め方5軸と認知・感情・関係・価値判断7軸を通る
> **最大12問の校正セッション**にする。ただし中断とスキップは常に残す。
...
| 時点 | 上限 | 根拠 |
|---|--:|---|
| 初回起動 | **一度だけ最大12問。**スキップ・中断可（FR-24） | 一人を深く知る。閉じた後は自動再表示せず、人物アイコンから再開できる |
...
> 12問は心理診断の尺度ではなく、異なる関わり方を一度ずつ選ぶ製品上の上限である。

#### 初回表示と、属性を扱う境界（2026-09-05）

- DBを開いた直後、モデル取得より前に、新規利用者へ一度だけ質問シートを開く
- 表示済みは版付きの `UserDefaults` キーに記録する。無回答で閉じても次回は強制しない
- 既に `user_traits` がある利用者には自動表示せず、移行済みとして表示済みにする
- 生年月日・血液型などの類型から性格を断定しない。**本人が類型をどう受け止めるか**を
  仮説の入口として訊き、実際の選択と会話を根拠にする
- 血液型が説明する性格差は大規模調査で0.3%未満だった
  （[Nawata 2014](https://pubmed.ncbi.nlm.nih.gov/25016835/)）。したがって診断点数には使わない
...
#### v4: 訂正の証拠（`trait_evidence` / 2026-09-21）

**v3 までは、訂正が「どの答えに対して押されたか」を捨てていた。**
`turnID` は画面の受領にだけ入り、DB に届いていなかった。**像に残るのは軸・向き・定型文だけで、
焼く材料になる実例は1件も無かった。** 動くし、緑になるし、確信度も上がる ──
**焼く段になって初めて気づく壊れ方**である。
...
- **像の文は方針、こちらは実例である。重みへ焼けるのは実例だけ** ── 第一号（`docs/ADAPTER_01.md`）が
  20組の問いと答えで出来ていたのと同じで、**方針の文を20回見せても振る舞いは変わらない**
- **`messages.id` を参照せず、本文を写し取る。** 会話は消せるので、参照にすると
  **「確信度 0.9 だが実例は0件」**という根拠を辿れない像が残る（NFR-12）。
  また画面の1つの答えは、ツールの往復を挟むと `messages` の複数行に分かれる
- **生成中・中断・失敗の答えは採らない。** 採ると、焼かれるのは内容ではなく
  **「途中で切れること」**になる（14.13b で短さが一緒に焼けたのと同じ型）。
  **それでも像は記録する** ── 実例が採れないことは、押した事実まで捨てる理由にならない
- **像と同じトランザクションで書く。** 分けると v3 までの状態（確信度だけ上がる）が再発しうる
- **`ON DELETE CASCADE`。** 実例には利用者の問いの本文が入る。**像を消して実例が残るなら、
  「消した」と言いながら、いちばん生々しい部分を残している**ことになる（FR-28）
- **過去の訂正には埋め戻さない**（v3 の `direction` と同じ）。
  どの答えに押されたかは、押された時点でしか分からない ── **推測で埋めるのは観測ではなく捏造である**

> **⚠ ここにあるのは「却下された答え」だけである。** 学習には**「こう答えるべきだった」**の側が要る。
> それをどう作るか（像の文を system に入れて生成し直し、system 抜きで焼く ＝ 第一号と同じ型）は
> **まだ決めていないし、測ってもいない。** 本節が保証するのは**材料を捨てなくなったこと**までである。
...
| **⑨** | **問数の的中率が未測定** | 最大12問は診断尺度ではなく製品上の上限。効果は14.12節で比較する |
...
#### 申し送りに書く前に、**その場で取り直す**（R11 の系）

**R11 は「引用は検算ではない」を数字について言っている。同じことが申し送りにも起きる。**

**2026-08-23 の実例:**

1. 検証役が「DESIGN の記述が実物と食い違っている」と報告した
2. **監督役が直した**（`3330beb`）
3. **そのあとで検証役が、申し送り（`b4c5903`）に「まだ偽である」と書いた**

**検証役は、自分の過去の観測を、コミットの瞬間に取り直さずに引き写していた。**
**しかも取り直しの費用は `grep` 一発だった。**

> **申し送りに書く前に、その場で取り直すこと。**
> **「いつ時点の観測か」を書くのは、取り直せなかったときの作法であって、取り直しの代わりにはならない。**

**⚠ 監督役はこのとき「書いた時点の文脈が古かったのだろう」と好意的な説明を当てた。**
**`git log` を確かめた検証役のほうが正確だった** ── 順序は「報告 → 修正 → その後に『まだ偽』と記録」である。
**確かめずに当てた説明は、それ自体が新しい未検算の主張である。**

> **さらに: これは「庇う方向」の誤りだった。** 検証役の指摘 ──
> **他人の誤りに好意的な説明を当てるのも、確かめずに埋めることの一種である。**
> **R7 は「合計が内訳と合わないとき、推測で埋めるな」と言っている。説明も同じである。**
> **この日の誤りはほとんどが自分に不利な方向へ倒れていた**（監督役が自分の数字の甘さを、
> 実装役が自分の器の穴を、検証役が自分の分母の取り違えを、それぞれ先に出した）。
> **庇う方向の誤りは、その並びの中では珍しい形であり、だから見つけにくい。**

#### 「逆向きの欠陥」を数える —— **直した結果、打ち切れなくなっていないか**

**2026-08-23、モデル取得の誤検知（`Progress` だけを見て健全な取得を打ち切っていた）を直す作業で、
修正案そのものに逆向きの欠陥が2件出た。**

| # | 何が起きたか |
|---|---|
| 1 | **待ちに天井が無くなった。** ディスクが「変化した」だけで時計が戻るので、**無関係な増減で永久に待つ** |
| 2 | **`finished` が、壊れていると分かっている信号にぶら下がっていた** ── `Progress` 由来なので、**`Progress` が更新されない状況では永久に立たない** |

**2 が本質的である。** 取得が完了しても `finished` が立たず、一時ファイルが片付いてディスクも止まり、
**重みの展開中に「両信号が無風」として完了した取得を打ち切る。**

> **事故が消えたのではなく、起きる場所が「取得中」から「展開中」へ移っただけだった。**
> **`Progress` の故障は、`Progress` に依存しない信号でしか救えない。**

**規則として:**

> **誤検知を直したら、「今度は検知できなくなっていないか」を必ず数えること。**
> **陰性対照（両方動いている＝健全）だけでなく、陽性対照（本当に死んでいる＝打ち切れる）を必ず置く**（R4・R5）。
> **そして直した機構が依存している信号が、元の故障と同じ信号でないことを確かめること。**

#### 閾値を足すときは、**新しい数字を作る前に、既にある数字を探す**

**欠陥1 の修正で「時間の絶対上限 `limit × N`」が提案されたが、採らなかった。**
**1ファイルが巨大なとき `Progress` が長時間沈黙するのは正常**であり、
時間で切ると**元の故障の下で、正常な取得を今度は時間で殺す。**

**代わりに、`downloadStallTimeout` の型コメントが既に持っていた数字を使った:**

> **実測 20MB/s の 1/1000（20KB/s）まで落ちた回線でも、60秒あれば 1.2MB は増える。**

**「変化したか」ではなく「1.2MB 以上動いたか」で時計を戻す。**
**瀕死の回線でも本物なら必ず戻り、雑音は 1.2MB を動かさない。**

> **⚠ 差をどこから測るかが本質である。** 1.2MB は **「60秒あれば」** の量であって、1回の観測ごとの量ではない。
> **直前の観測との差**に当てると、20KB/s の回線を5秒ごとに観測して1回100KB ──
> **永久に 1.2MB へ届かず、この閾値が守るはずだった当のものを殺す。**
> **最後に時計を戻した時点との差**に当てること。
>
> **⚠ そして「天井が要らなくなる」は言い過ぎだった**（監督役の誤り。検証役が訂正した）。
> **1.2MB は雑音を落とすフィルタであって、天井ではない。**
> 同じ観測対象で 1.2MB 以上の増減が繰り返し起きれば（例: 同じ一時ディレクトリで別の取得が動く）、
> **時計は戻り続ける。窓は狭まるが、閉じてはいない。**
> **正しくは「天井が要る条件が十分に狭まった」である** ── 「上限は考えなくてよい」と読ませないこと。

> **新しい定数を作ると、片方を変えたときにもう片方が古くなる**（R10）。
> **既にある数字で切れるなら、そちらを参照すること。**
